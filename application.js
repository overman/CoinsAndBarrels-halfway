dojo.provide("coinsAndBarrels");
dojo.require("dojox.timing._base");
dojo.require("dijit.Dialog");
dojo.require("dijit.form.Button");
dojo.require("welcomeAndCreditsData");

dojo.declare('coinsAndBarrels', null, {
    constructor: function() {    //load screen
        ...
        this._WCData = new welcomeAndCreditsData();
        this._exitedInstructions = false;
        var gameDataRequest = {
            url : "coins_and_barrels.json",
            handleAs : 'json',
            preventCache: true,
            error: function(error) {console.log("THERE WAS AN ERROR", error);}
        };
        var gameDataDef = dojo.xhrGet(gameDataRequest);
        gameDataDef.then(dojo.hitch(this, function(data) {
            this.goodSounds = data.Good_Sounds;
            this.badSounds = data.Bad_Sounds;
            this.responseTime = data.Reaction_Time;
            this._gameLength = data.Game_Time_Length;
            this.gamePlayImages = data.Game_Play_Images;
            this.mainImageNode = dojo.byId("mainImage");
            this.mainImageNode.src = this._oneOf(this.gamePlayImages);
            dojo.removeClass(this.mainImageNode, 'hidden');
            this.rewardSounds = data.Reward_Sounds;
            this.missedGoodMessages = data.onMissGood_Messages; 
            this.wrongHitRight = data.onHitRightButWrong_Messages;
            this.wrongHitLeft = data.onHitLeftButWrong_Messages;
            this.badWasHit = data.onBadHit_Messages;
            this.pauseImages = data.Pause_Screen_Images; 
            this.pauseMessages = data.Pause_Screen_Messages;   
            this.endImages = data.End_Game_Images;    
            this.instructions = data.Instructions;
            this.endSounds = data.End_Game_Sounds;
            
            var def = uow.getAudio({defaultCaching: true});    //get JSonic
            return def;    
        })).then(dojo.hitch(this, function(audio) {
            this._audio = audio;
            this._putStuffIn(); //poorly phrased, for sound credits
    ...
    readOffSounds: function() {
        var goodSoundsClone = dojo.clone(this.goodSounds);
        var badSoundsClone = dojo.clone(this.badSounds); 
        if(!this.exitedInstructions) {           
            this._audio.say({text: "Here are the good sounds. You want to hit these."});
            while (goodSoundsClone.length) { //just queuing up
                //don't add anymore to queue if exited. queue cleared on exit
                if (this.exitedInstructions) {} 
                else{
                    var sound = goodSoundsClone.pop();
            ...
            if(!this.exitedInstructions){// then do the bad ones
                ...
                while (badSoundsClone.length) { //just queuing up
                    //don't add anymore to queue if exited. queue cleared on exit
                    if (this.exitedInstructions) {} 
                    else{
                        var sound = badSoundsClone.pop();
    ...
    //  for the sound credits. adds the HTML for credits. Will not be in final version as
    //  credits will be elsewhere
    _putStuffIn: function() {
        dojo.byId('creditsText').innerHTML = this._WCData.creditsData;
    },
    ...
    _pause: function() {
        ...
        {
            this.mainImageNode.src = this._oneOf(this.pauseImages);
        }));
    },
